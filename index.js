...
function DDatabase (createStorage, key, opts) {
  if (!(this instanceof DDatabase)) return new DDatabase(createStorage, key, opts)
...
DDatabase.revelationKey = crypto.revelationKey

DDatabase.prototype[inspect] = function () {
  return 'Hypercore(\n' +
    '  key: ' + (this.key && pretty(this.key)) + '\n' +
    '  revelationKey: ' + (this.revelationKey && pretty(this.revelationKey)) + '\n' +
    '  opened: ' + this.opened + '\n' +
...
Object.defineProperty(DDatabase.prototype, 'remoteLength', {
  enumerable: true,
  get: function () {
    var len = 0
...
DDatabase.prototype.replicate = function (opts) {
...
DDatabase.prototype.ready = function (onready) {
  this._ready(function (err) {
    if (!err) onready()
  })
}

DDatabase.prototype.update = function (len, cb) {
...
DDatabase.prototype._writeStateReloader = function (cb) {
  var self = this
  return function (err) {
    if (err) return cb(err)
...
DDatabase.prototype._open = function (cb) {
  var self = this
...
DDatabase.prototype.download = function (range, cb) {
...
DDatabase.prototype.undownload = function (range) {
  if (typeof range === 'number') range = {start: range, end: range + 1}
  if (!range) range = {}
...
DDatabase.prototype.digest = function (index) {
  return this.tree.digest(2 * index)
}

DDatabase.prototype.proof = function (index, opts, cb) {
...
DDatabase.prototype._readyAndProof = function (index, opts, cb) {
  var self = this
  this._ready(function (err) {
    if (err) return cb(err)
...
DDatabase.prototype.put = function (index, data, proof, cb) {
  if (!this.opened) return this._readyAndPut(index, data, proof, cb)
  this._putBuffer(index, this._codec.encode(data), proof, null, cb)
}

DDatabase.prototype.cancel = function (start, end) {  // TODO: use same argument scheme as download
  if (!end) end = start + 1
...
DDatabase.prototype._cancel = function (start, end) {
  var i = 0
...
DDatabase.prototype.clear = function (start, end, opts, cb) { // TODO: use same argument scheme as download
...
DDatabase.prototype.signature = function (index, cb) {
...
DDatabase.prototype.verify = function (index, signature, cb) {
  var self = this
...
DDatabase.prototype.rootHashes = function (index, cb) {
  this._getRootsToVerify(index * 2 + 2, {}, [], cb)
}

DDatabase.prototype.seek = function (bytes, opts, cb) {
...
DDatabase.prototype._seek = function (offset, cb) {
...
DDatabase.prototype._readyAndSeek = function (bytes, opts, cb) {
  var self = this
  this._ready(function (err) {
    if (err) return cb(err)
...
DDatabase.prototype._getBuffer = function (index, cb) {
  this._storage.getData(index, cb)
}

DDatabase.prototype._putBuffer = function (index, data, proof, from, cb) {
...
DDatabase.prototype._readyAndPut = function (index, data, proof, cb) {
  var self = this
  this._ready(function (err) {
    if (err) return cb(err)
...
DDatabase.prototype._write = function (index, data, nodes, sig, from, cb) {
...
DDatabase.prototype._writeAfterHook = function (index, data, nodes, sig, from, cb) {
  var self = this
...
DDatabase.prototype._writeDone = function (index, data, nodes, from, cb) {
...
DDatabase.prototype._verifyAndWrite = function (index, data, proof, localNodes, trustedNode, from, cb) {
  var visited = []
...
DDatabase.prototype._verifyRootsAndWrite = function (index, data, top, proof, nodes, from, cb) {
  var remoteNodes = proof.nodes
...
DDatabase.prototype._getRootsToVerify = function (verifiedBy, top, remoteNodes, cb) {
...
DDatabase.prototype._announce = function (message, from) {
  for (var i = 0; i < this.peers.length; i++) {
...
DDatabase.prototype.get = function (index, opts, cb) {
...
DDatabase.prototype._readyAndGet = function (index, opts, cb) {
  var self = this
  this._ready(function (err) {
    if (err) return cb(err)
...
DDatabase.prototype.getBatch = function (start, end, opts, cb) {
...
DDatabase.prototype._getBatch = function (start, end, opts, cb) {
...
DDatabase.prototype._readyAndGetBatch = function (start, end, opts, cb) {
  var self = this
  this._ready(function (err) {
    if (err) return cb(err)
...
DDatabase.prototype._updatePeers = function () {
  for (var i = 0; i < this.peers.length; i++) this.peers[i].update()
}

DDatabase.prototype.createWriteStream = function () {
  var self = this
...
DDatabase.prototype.createReadStream = function (opts) {
  if (!opts) opts = {}
...
// TODO: when calling finalize on a live ddb write an END_OF_FEED block (length === 0?)
DDatabase.prototype.finalize = function (cb) {
  if (!this.key) {
...
DDatabase.prototype.append = function (batch, cb) {
  this._batch(Array.isArray(batch) ? batch : [batch], cb || noop)
}

DDatabase.prototype.flush = function (cb) {
  this._batch([], cb)
}

DDatabase.prototype.close = function (cb) {
  var self = this
...
DDatabase.prototype._appendHook = function (batch, cb) {
  var self = this
...
DDatabase.prototype._append = function (batch, cb) {
...
DDatabase.prototype._readyAndAppend = function (batch, cb) {
  var self = this
  this._ready(function (err) {
    if (err) return cb(err)
    self._append(batch, cb)
...
DDatabase.prototype._readyAndCancel = function (start, end) {
  var self = this
  this.ready(function () {
...
DDatabase.prototype._pollWaiting = function () {
...
DDatabase.prototype._syncBitfield = function (cb) {
...
DDatabase.prototype._roots = function (index, cb) {
...
function defaultStorage (dir) {
  return function (name) {
    return DWREF(name, {directory: dir})
  }
}
